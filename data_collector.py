...
def get_leetcode_stats(username):
    ...
    try:
        response = requests.post(LEETCODE_GRAPHQL_URL, json=payload, timeout=REQUEST_TIMEOUT)
...
def update_progress_data():
    ...
    if not os.path.exists(CSV_FILE):
        with open(CSV_FILE, 'w', newline='', encoding=CSV_ENCODING) as f:
            ...
            headers = ['timestamp', 'username', 'total_solved', 'easy_solved', 'medium_solved', 'hard_solved']
    ...
    with open(CSV_FILE, 'a', newline='', encoding=CSV_ENCODING) as f:
        ...
        for username in USERNAMES:
            ...
            if stats is not None:
                ...
                print(f"  - {username}: {stats['total']} задач (E:{stats['easy']}, M:{stats['medium']}, H:{stats['hard']}). Запись добавлена.")
            else:
